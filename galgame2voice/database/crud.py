...
4. 始终以四季夏目的身份回复，不要解释设定、不要跳出角色。"""
    ...
    if count == 0:
        presets = [
            ("gemini", "Google Gemini", "https://generativelanguage.googleapis.com/v1beta/openai", "gemini-2.0-flash", "", 1),
            ("openai", "OpenAI", "https://api.openai.com/v1", "gpt-4o", "whisper-1", 0),
            ("deepseek", "DeepSeek", "https://api.deepseek.com", "deepseek-chat", "", 0),
            ("anthropic", "Anthropic Claude", "https://api.anthropic.com/v1", "claude-3-5-sonnet-latest", "", 0),
            ("xai", "xAI (Grok)", "https://api.x.ai/v1", "grok-2", "", 0),
            ("glm", "智谱 GLM", "https://open.bigmodel.cn/api/paas/v4", "glm-4-plus", "", 0),
            ("qwen", "通义千问 (Qwen)", "https://dashscope.aliyuncs.com/compatible-mode/v1", "qwen-max", "qwen-audio-asr", 0),
            ("custom", "自定义 / 本地模型 (Ollama / vLLM)", "http://127.0.0.1:11434/v1", "llama3.3", "", 0)
        ]
        for p in presets:
            await conn.execute("""
